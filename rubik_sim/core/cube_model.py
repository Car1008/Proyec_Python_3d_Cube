...
class CubeModel:
    ...
    def apply_move(self, move: str):
        """
        Soporta: U D L R F B y slices: E M S
        Sufijos: ' y 2
        """
        move = move.strip()
        if not move:
    ...
    def _rot_x(v, turns):
        x, y, z = v
        turns %= 4
        if turns == 0: return (x, y, z)
        if turns == 1: return (x, -z, y)
    ...
    def _rot_z(v, turns):
        ...
        turns %= 4
        if turns == 0: return (x, y, z)
        if turns == 1: return (y, -x, z)
        if turns == 2: return (-x, -y, z)
        return (-y, x, z)

    def _rotate_layer(self, axis: str, layer_value: int, turns: int):
        """
        axis: 'x'/'y'/'z'
        layer_value: -1/0/1
        turns: +1 o -1 o 2 etc (90° steps)
        """
        # Normalizamos turns a [0..3]
        t = turns % 4

        # Copia del estado actual
        old = {f: self.state[f][:] for f in self.FACES}
        new = {f: self.state[f][:] for f in self.FACES}
